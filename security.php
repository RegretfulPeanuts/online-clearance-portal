...
session_start();
	     ...
	     if(!isset($_SESSION['security']))
   {
    ...
    }else
	{
	  ...
	  $select=$connection->prepare("SELECT * FROM security WHERE `regno`='$regno' ");
		 $select->execute();
		 $num=$select->rowCount();
		 if($num==0)
		 {
            $insert=$connection->prepare("INSERT into security(`regno`, `payment`) VALUE('$regno' , 'security clearance')");
		    $insert->execute();
			}
			...
			if(isset($_POST['submit_security']))
			{
				...
				$_SESSION['not']="no";
             	$update=$connection->prepare("UPDATE `security` SET `payment`='cleared' WHERE `regno`='$regno'");
					$update->execute();		
                    $update=$connection->prepare("UPDATE `student_info` SET `security_c`='cleared' WHERE `regno`='$regno'");
					$update->execute();
					$_SESSION['security']="due";
            ...
            $select_due=$connection->prepare("SELECT * FROM security where `regno`='$regno' ");
            $select_due->execute();
	}	
	...
	<body class="landing">
		...
		<!-- One -->
			<section id="one" class="wrapper style1 specia">
				...
				<input type="submit"  name="submit_security" value="SUBMIT" id="submit">
			</form>
				</div>
			</section>
