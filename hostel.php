...
session_start();
	     ...
	     if(!isset($_SESSION['hostel']))
   {
    ...
    }else
	{
	  ...
	  $select=$connection->prepare("SELECT * FROM hostel WHERE `regno`='$regno' ");
		 $select->execute();
		 $num=$select->rowCount();
		 if($num==0)
		 {
            $insert=$connection->prepare("INSERT into hostel(`regno`, `payment`) VALUE('$regno' , 'hostel clearance')");
		    $insert->execute();
			}
			...
			if(isset($_POST['submit_hostel']))
			{
				...
				$_SESSION['not']="no";
             	$update=$connection->prepare("UPDATE `hostel` SET `payment`='cleared' WHERE `regno`='$regno'");
					$update->execute();				
					$update=$connection->prepare("UPDATE `student_info` SET `hostel_c`='cleared' WHERE `regno`='$regno'");
					$update->execute();
					$_SESSION['hostel']="due";
	...
	<body class="landing">
		...
		<!-- One -->
			<section id="one" class="wrapper style1 specia">
				...
				<input type="submit"  name="submit_hostel" value="SUBMIT" id="submit">
			</form>
				</div>
			</section>
