...
session_start();
	     ...
	     if(!isset($_SESSION['library']))
   {
    ...
    }else
	{
	  ...
	  $select=$connection->prepare("SELECT * FROM library WHERE `regno`='$regno' ");
		 $select->execute();
		 $num=$select->rowCount();
		 if($num==0)
		 {
            $insert=$connection->prepare("INSERT into library(`regno`, `payment`) VALUE('$regno' , 'library clearance')");
		    $insert->execute();
			}
			...
			if(isset($_POST['submit_library']))
			{
				...
				$_SESSION['not']="no";
             	$update=$connection->prepare("UPDATE `library` SET `payment`='cleared' WHERE `regno`='$regno'");
					$update->execute();
					
                $update=$connection->prepare("UPDATE `student_info` SET `library_c`='cleared' WHERE `regno`='$regno'");
					$update->execute();
                $_SESSION['library']="due";
            ...
            $select_due=$connection->prepare("SELECT * FROM library where `regno`='$regno' ");
            $select_due->execute();
	}
	...
	<body class="landing">
		...
		<!-- One -->
			<section id="one" class="wrapper style1 specia">
				...
				<input type="submit"  name="submit_library" value="SUBMIT" id="submit">
			</form>
				</div>
			</section>
